...
Every durable claim — `state/notes[-persona].jsonl` and `state/memories-{persona}.jsonl` — carries where it came from, so a retrieved memory can answer *"did I see this, was I told it, or did I make it up?"*.

Six kinds, each with a **confidence ceiling clamped on write and again on read**: `observation` (1.0, wander scene descriptions — the only genuine perception writer) · `verification` (1.0, no writer yet) · `report` (0.9, voice-loop remember, tool-research) · `inference` (0.6, no writer yet) · `narrative` (0.5, consolidation, tool-compose, tool-story, `auto_remember`, px-mind's `remember`) · `unknown` (0.3, everything written before this existed). **The ordering between ceilings is the safety property; the numbers are calibration.** SPARK's store is overwhelmingly `narrative` — its own prose about itself — and unlabelled that reads back exactly like something witnessed.

- **The model never types its own claims.** `validate_action` sets `PX_NOTE_KIND` as a constant per call path, never from LLM params, and `_parse_memory_array` drops any `kind`/`confidence`/`provenance`/`supersedes` the consolidating model emits. A writer able to stamp its own speculation `observation` would launder it into perception.
- **Ceilings deliberately do *not* live in `spark_config.py`** — that module is the self-evolution whitelist's primary target, and SPARK must not be able to propose raising its own confidence ceilings.
- **Strict on write, lenient on read**: `make_provenance()` raises on a bad kind (our writers pass literals, so it's a caller bug); `read_provenance()` never raises and degrades anything it can't vouch for to `unknown`. Same reasoning as `health.py` — reporting must not kill the daemon it reports on.
- **Legacy records stay readable at `unknown`.** Their coarse `source` string is kept ("consolidation (legacy, provenance not recorded)") but never promoted into a kind: a stored string is not evidence of how a claim was produced.
- **Correction never deletes.** A newer record names an older one in `provenance.supersedes`; `apply_supersessions()` marks the old one `superseded_by` **on a copy**, and `read_provenance()` discounts its confidence (derived at read time, like health status). Relevance retrieval skips superseded records; `mode="recent"` still shows them, marked — SPARK should be able to remember having been wrong.
- Reflection injects each memory as `- <text> [<where it came from>]`; `load_notes(..., with_provenance=True)` does the same for the raw-notes fallback.
- **Speech must not erase epistemic distance.** `bin/tool-recall` is the one place a durable claim is spoken to a human, so it renders per-kind: "I remember seeing…" / "you told me…" / "I thought…", with `unknown` bare ("I remember: …") because it has no distance to report. Kinds are qualified **per note** in the multi-note form — one shared lead-in would relabel every note with a single kind. Its payload keeps `notes` as a list of strings and adds `provenance` alongside.
- **All notes writers key the text `note`.** `mind.load_notes` and `tool-recall` both read `record["note"]`; `tool-story` used to write `text`, so finished stories were saved to long-term memory and then invisible to everything that reads it.

**Retrieval never pads.** `retrieve_memories()` returns *only* topical matches — fewer than `n` matches returns fewer than `n` records. It used to fill spare slots with recent memories, and by the time they reached the prompt a filler was indistinguishable from a real hit. Recency-only retrieval still exists but must be asked for explicitly (`mode="recent"`). `importance` deliberately does not affect ranking — it is assigned by the consolidating model to its own output. The `elif has_store` branch in `mind.py` matters for the same reason: a store that returned *nothing relevant* must not fall through to the raw-notes tail, which would re-open the hole by another door. Only a persona with **no store at all** (or a retrieval *failure*, which tells us nothing) gets the notes fallback.
