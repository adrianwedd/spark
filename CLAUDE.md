...
- **The capture path must never block on IO to log (#283).** `pxh.mic_stream`'s reader thread is what keeps `arecord` drained, and it logs drops and overruns — while holding the stream's condition lock, which is the lock every consumer of that stream waits on. `bin/px-wake-listen` therefore hands its lines to `pxh.logging.StallProofLineWriter` (one background writer thread, bounded queue, drops the *oldest* line with a visible marker, `flush()` on exit): a log write that blocks on a stalled card or a stuck journald socket otherwise turns a short hiccup into an ALSA overrun, which is logged by the same blocked path. The invariant is that **a daemon's evidence must not be produced by the machinery that is losing the data** — same reason px-alive's health write moved to a background thread.
- **ALSA's capture buffer is `mic_stream.DEFAULT_ALSA_BUFFER_S` (4 s), not the old 0.37 s.** The ring buffer below it is 10 s of app-side slack, but ALSA's own buffer is what overruns when the reader stalls: at `chunk_frames * 8` (2048-frame periods at 44.1 kHz) it absorbed 0.37 s, which is why production overruns measured seconds of loss. Every overrun line now carries `reader_gap_last_ms_at` / `reader_gap_max_ms_at` (windowed per capture) so a stalled *reader* is distinguishable from arecord losing the device itself.
...
**The park's lease read runs off the loop's thread (#287).** The park loop's product is *liveness* — a heartbeat plus an `EXTEND_TIMEOUT_USEC` every `LEASE_RECHECK_S` — and reading `state/gpio_lease.json` is SD-card IO on the card #247 measures stalling for tens of seconds. On the loop's own thread, a stalled read stopped the beats exactly when the filesystem stalled and systemd killed a park that was never hung (26 `start operation timed out` events since 2026-08-26, `NRestarts=39`). `_PendingLeaseRead` therefore owns the read on a daemon thread; the loop keeps beating and consumes the answer when it lands, and the heartbeat carries `lease_read_pending_ms` / `lease_read_last_ms` so a park waiting on a stalled read is distinguishable from a wedged daemon. Reordering the extension ahead of the read is deliberately *not* the fix on its own: the filed event ran 82 s against a 30 s extension, so beats must keep landing during the block. An invalid lease record is still fail-closed (`_read()` raises, the daemon refuses to start) — only a *missing* or *expired* lease lets the park end.
...
**HA is an evidence source, so losing it is a stated fact (#191).** It is the only source for presence, on-call and hot-mic state; when it is unreachable the policy layer correctly fails *open*, which means the operationally important event is that something stopped being observable while everything still looked fine. Three things now say so instead of a timeout in a log: `pxh.health` carries an `"ha"` component (window 900 s = 3× `mind.HA_INTERVAL_S`) so `read_health()`/the operator board reads degraded or stale, reported as its own `checks["perception"]` block but **kept out of the API's liveness rollup** — `/api/v1/health` answers "is the device serving", and a 503 here takes the tunnel health check with it, so a home-network fault must not read as the robot being down (same rule as the consolidation component); `awareness["ha_perception_unavailable"]` names *why* the modality is missing, because an absent `ha_presence` and "HA could not be reached" look identical to every consumer otherwise; and a fail-open verdict is named — `policy.on_call_suppression(awareness)` returns `(suppressed, evaluable)`, an allowed verdict carries `unevaluated=("on_call",)`, and `validate_action` records `policy_fail_open`. Both dispatchers (the interactive sink gate and mind's autonomous expression loop) call that one helper: a second opinion about what "no evidence" means is how the two would drift. **Do not make the fail-open invisible again** — "no suppression fired" and "suppression could not be evaluated" are different claims, and only one of them is safe to read as "nothing needed suppressing". The posture itself is unchanged: a missing signal is not evidence of a call, and suppressing speech on every HA outage would silence SPARK for a network fault.
