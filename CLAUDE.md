...
- **Memory consolidation**: nightly Haiku pass (02:00–06:00 Hobart, ≤2 attempts/day, state/consolidation_meta.json) distills the last 24h of thoughts into state/memories-spark.jsonl; reflection retrieves the top-3 relevant memories by keyword/tag overlap. `consolidate` is a resident-brain kind (`ask_brain`), and the window's start hour is the same Hobart hour as `brain_daemon.NIGHTLY_RECYCLE_HOUR` — not a coincidence to design around but a real collision (#278): an attempt that lands mid-recycle burns lock contention instead of an answer, and 2 such races exhausts the whole day's attempts with nothing written. During the window's first hour only, `maybe_consolidate` checks `brain.recycle_landed_today()` (reads brain_daemon's `recycle_state.json`) and defers without spending an attempt until today's reset has landed; past that hour it proceeds regardless, so a recycle still waiting on a busy brain doesn't cost the whole day.
- **Goal persistence** in state/intention-spark.json (7-day expiry, one active at a time).
