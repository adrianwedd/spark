...
| `run-voice-loop-tier` | Launches the voice loop on the cognition tier. Sets `PX_VOICE_BACKEND=tier`, so voice turns are one direct API call (`pxh.m5`) — no session, no mailbox, no CLI. Uses `docs/prompts/voice-system.md` as the system prompt. |
