...
| `px-wander` | Guarded obstacle avoidance/exploration. Before the first live run on a surface, place all three grayscale sensors over that floor and run `bin/px-wander --calibrate-cliff`; live calibration self-elevates, marks `state/exploring.json` before yielding `px-alive`, rejects the ADC power-on latch, and cleans up the marker on exit. |
