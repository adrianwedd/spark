...
def choose_target(last_heard: dict | None, available: set[str] | None,
    ...
    def _ok(entity: str | None) -> bool:
        return bool(entity) and (available is None or entity in available)
...
def fetch_available(entities: list[str], ha_base: str, ha_token: str,
                    timeout: float = 3.0) -> set[str] | None:
    got: dict[str, str | None] = {e: _get_state(e, ha_base, ha_token, timeout)
                                  for e in entities}
...
**Interfaces:**
- Consumes: `spark_config.SPEAKER_ROOMS`, existing `_sanitize_chat_text()` (api.py:1377), `atomic_write` from `pxh.state`.
...
```python
def test_public_chat_area_writes_last_heard(client, tmp_state_dir, monkeypatch):
    ...
    data = _json.loads((api_mod.STATE_DIR / "last_heard.json").read_text())
    assert data["room"] == "office"
    assert "ts" in data


def test_public_chat_unknown_area_writes_nothing(client):
    from pxh import api as api_mod
    lh = api_mod.STATE_DIR / "last_heard.json"
    if lh.exists():
        lh.unlink()
    resp = client.post("/api/v1/public/chat",
...
(Match the file's actual import names — it may already import `datetime`/`timezone`/`json`; `STATE_DIR` already exists in api.py. If `atomic_write`'s signature differs, follow `pxh/state.py`.)
...
- Produces: after a successful cast — `media_player/volume_set` with the pre-cast `volume_level`, plus `media_player/media_play` iff the snapshot state was `"playing"`. No restore on `suppressed`/`error`/`dry` (those paths return before casting).
...
def test_tool_announce_restores_volume_and_playback(isolated_project):
    ...
    assert play    # snapshot said "playing" → best-effort media_play issued
```

And the negative case:

```python
def test_tool_announce_idle_snapshot_skips_media_play(isolated_project):
    # same recipe, get_state {"state": "idle", "attributes": {"volume_level": 0.7}}
    # assert volume_set fired with 0.7 and media_play did NOT
```

(Write it out fully — copy the recipe above, flip the state and the assertions.)
...
```python
        ...
        # Interrupt-and-restore (spec §Interrupt-and-restore): volume always,
        # playback best-effort. Skipped entirely when nothing was cast.
        if cast_ok:
            time.sleep(float(result.get("duration_s") or 0) + 1.0)
            for entity in cast_ok:
                ...
                if snap.get("state") == "playing":
                    _ha_service("media_play", {"entity_id": entity},
                                cfg.HA_BASE_URL, ha_token)
```
...
**Interfaces:**
- Consumes: `pxh.speaker_router.resolve_speaker()` (Task 2), `bin/tool-announce` JSON contract (`status`: `ok` / `suppressed` / `error` / `dry`), `spark_config.ANNOUNCE_ENABLED`.
...
```python
    ...
    def _try_nest_route(text: str) -> dict | None:
        ...
        to fall back to the onboard path.
        """
...
- [ ] **Step 5: Run to verify pass** — `python -m pytest tests/test_tools.py -k "tool_voice or announce" -q`; expected: all pass. Then full suite: `python -m pytest -q -m "not live"`.

- [ ] **Step 6: Commit**

```bash
git add bin/tool-voice src/pxh/spark_config.py tests/test_tools.py
