...
def _render(headline: str, body: str, variant: str, when: str | None) -> Path:
    ...
    y = 150
    for line in _wrap(draw, headline, f_head, text_w, 2):
    ...
    for line in _wrap(draw, body, f_body, text_w, MAX_BODY_LINES):
...
```yaml
          - action: rest_command.announce_card
            response_variable: card_response
            data:
              headline: "{{ headline }}"
              message: "{{ message }}"
              variant: "{{ variant | default('task') }}"
          - variables:
              relay: "{{ card_response }}"
