...
def _tmux(*args: str, timeout: float = 5.0) -> str | None:
...
def _tmux_via_pty(*args: str, timeout: float = 5.0) -> bool:
    ...
    """
    global _last_error
    cmd = "tmux -S " + shlex.quote(SOCKET) + " " + " ".join(shlex.quote(a) for a in args)
    try:
        proc = subprocess.run(
...
def session_exists() -> bool:
    return _tmux("has-session", "-t", SESSION) is not None


def brain_pane() -> str | None:
    """Pane ID of the session's first pane — the injection target (workaround 1)."""
    out = _tmux("list-panes", "-t", SESSION, "-F", "#{pane_id}")
    if not out:
        return None
...
def pane_ready() -> bool:
    ...
    pane = _tmux("capture-pane", "-t", SESSION, "-p")
    return pane is not None and READY_GLYPH in pane


def ensure_session(timeout_s: float = STARTUP_TIMEOUT_S) -> bool:
    """Start the brain if it is not already up. Idempotent."""
    if session_exists():
        return True
...
def inject(text: str) -> bool:
    ...
    if not clean:
        ...
        return False
    target = brain_pane() or SESSION
    if not _tmux_via_pty("send-keys", "-t", target, "-l", clean):
        return False
    time.sleep(_ENTER_GAP_S)
    return _tmux_via_pty("send-keys", "-t", target, "Enter")
...
def kill_session() -> bool:
    return _tmux("kill-session", "-t", SESSION) is not None
...
class HolderClient:
    ...
    def start(self) -> None:
        ...
        attach = (
            ...
            f"exec tmux -S {shlex.quote(SOCKET)} attach-session -r -t {shlex.quote(SESSION)}"
        )
        try:
