...
def test_first_fix_outside_the_enter_radius_waits_for_confirmation():
    """One far fix is a claim about one GPS sample; a tracker at home produced
    one at ±100 m accuracy on 2026-09-17 and the next good fix greeted it."""
    assert presence.latch_at_home(1.4, ACC, None) == (None, presence.PENDING_REASON)
    assert presence.latch_at_home(1.4, ACC, None, 1) == (False, "away")
...
def test_exit_needs_the_wider_radius_and_a_confirmation():
    assert presence.latch_at_home(0.22, ACC, True) == (None, "hold-band")
    # Past the exit radius, but the first such fix only starts the run.
    assert presence.latch_at_home(0.35, ACC, True) == (None, presence.PENDING_REASON)
    assert presence.latch_at_home(0.35, ACC, True, 1) == (False, "exit")
    # ...and the same tracker re-entering is an arrival edge again.
    assert presence.latch_at_home(0.12, ACC, False) == (True, "enter")


def test_arrivals_are_never_confirmed():
    """The asymmetry is deliberate: nothing greets on a departure, so that is
    the only side where waiting is free."""
    assert presence.latch_at_home(0.10, ACC, False) == (True, "enter")
    assert presence.latch_at_home(0.10, ACC, None) == (True, "enter")
...
def test_accuracy_gate_is_judged_against_the_radius_in_play():
    ...
    assert presence.latch_at_home(0.40, 200.0, True) == (None, presence.PENDING_REASON)
    assert presence.latch_at_home(0.40, 200.0, True, 1) == (False, "exit")
...
def test_hold_reasons_are_declared_as_the_evidence_worthy_ones():
    ...
    assert presence.PENDING_REASON in presence.NEUTRAL_REASONS
    assert presence.PENDING_REASON not in presence.DECISION_REASONS
    assert presence.PENDING_REASON not in presence.SUPPRESSED_FLIP_REASONS
    assert presence.EXIT_CONFIRMATIONS >= 2
...
def test_boundary_jitter_series_moves_the_state_once():
    ...
    for distance in series:
        new_state, reason = presence.latch_at_home(distance, ACC, state, streak)
        streak = streak + 1 if reason == presence.PENDING_REASON else 0
